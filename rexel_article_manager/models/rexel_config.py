...
class RexelConfig(models.Model):
    ...
    _sql_constraints = [
        ('name_unique', 'unique(name)', 'Une seule configuration est autorisée !'),
    ]
    ...
    def get_config(self):
        ...
        if not config:
            config = self.create({'name': 'Configuration Rexel'})
        return config
    ...
    def _determine_unit_from_conditionnement(self, conditionnement, condition_label=None, api_unit=None):
        """
        ...
        2. PRIORITÉ 2 - Conditionnement : Vérifier la colonne conditionnement
           - TOU (touret) -> ML
           - MET (mètre) -> ML  
           - PIE (pièce) -> U
           - BOI / LOT / PAQ / SAC -> U
           
        3. DÉFAUT : Si rien trouvé -> U + message d'erreur avec la référence
        ...
        if api_unit:
            ...
            if api_unit_upper in ('PIECE', 'PCS', 'UNITE', 'UNITÉ', 'U', 'UN', 'PCE'):
                _logger.info(f"Unité API '{api_unit}' -> U")
                return ('U', None)
            
            # Conversion METRE -> ML
            if api_unit_upper in ('METRE', 'MÈTRE', 'ML', 'M', 'METER', 'METERS', 'METRES'):
                _logger.info(f"Unité API '{api_unit}' -> ML")
                return ('ML', None)
            
            # Autre unité API connue
            if api_unit_upper in ('KG', 'KILOGRAMME'):
                _logger.info(f"Unité API '{api_unit}' -> U (par défaut pour KG)")
                return ('U', None)
        
        # ========== PRIORITÉ 2 : Conditionnement ==========
        cond_str = str(conditionnement or '').strip().upper()
        label_str = str(condition_label or '').strip().upper()
        combined = f"{cond_str} {label_str}"
        
        # TOU (touret) ou MET (mètre) -> ML
        if 'TOU' in combined or 'TOURET' in combined:
            _logger.info(f"Conditionnement '{conditionnement}' contient TOU -> ML")
            return ('ML', None)
        
        if 'MET' in combined or 'METRE' in combined:
            _logger.info(f"Conditionnement '{conditionnement}' contient MET -> ML")
            return ('ML', None)
        
        # PIE (pièce) -> U
        if 'PIE' in combined or 'PIECE' in combined or 'PCS' in combined:
            _logger.info(f"Conditionnement '{conditionnement}' contient PIE -> U")
            return ('U', None)
        
        # BOI (boîte), LOT, PAQ (paquet), SAC -> U
        if 'BOI' in combined or 'BOITE' in combined or 'LOT' in combined or 'PAQ' in combined or 'SAC' in combined:
            _logger.info(f"Conditionnement '{conditionnement}' contient BOI/LOT/PAQ/SAC -> U")
            return ('U', None)
        ...
        # Conditionnement inconnu -> U + message d'erreur
    ...
    def get_product_prices_from_api(self, articles):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                _logger.info("=== RÉPONSE API productSalePrices ===")
                _logger.info(f"Réponse brute: {data}")
                ...
                if 'data' in data and 'productSalePricesExt' in data['data']:
                    ...
                    if isinstance(product_details, dict):
                        ...
                        _logger.info("productDetails est un dict (1 seul produit) -> converti en liste")
                    ...
                    for idx, product_data in enumerate(product_list):
                        ...
                        _logger.info(f"Produit {idx}: {product_data}")
                        ...
                        if idx < len(articles):
                            ...
                            if has_data:
                                ...
                                _logger.info(f"Article {article.reference_fabricant}: prix_base={prix_base_val}, prix_net={prix_net_val}, referenceRebate(montant)={product_data.get('referenceRebate')} -> remise_calculée={remise_calculee}%")
